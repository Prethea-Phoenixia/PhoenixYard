...
class Specie:
        ...
        with open(curvePath, "r") as file:
            ...
            if startLine is None:
                raise ValueError(
                    ...
                    + "Card data must start with line 'THERMO'"
                )
    ...
    def __str__(self):
        return (
            "Fit for "
            + self.specieName
            + " from {:.0f}K to {:.0f}K.".format(self.fitTlow, self.fitThigh)
        )
    ...
    def __call__(self, T, R="J/(molK)", C="p"):  # 1.987 for calorie based unit
        ...
        1.987 ... cal/molK| cal/mol K       | cal/mol         | cal/mol K
               ...
               T
        h(T) = ∫ Cp dT + h_ref = ΔΗ⁰_f @ 298.15 + (H⁰_T - H⁰_298.15)
              T_ref
        ...
        if T < self.fitTlow:
            raise ValueError(
                "Temperature below fit minimum ({:.0f}K) for this specie.".format(
                    self.fitTlow
                )
            )
            ...
            raise ValueError(
                "Temperature above fit maximum ({:.0f}K) for this specie.".format(
                    self.fitThigh
                )
            )
        ...
        for specie, count in LHS.items():  # Specie object, float
            for element, number in specie.formula.items():  # str, float
                if element in element_balance:
                    ...
                    raise ValueError(
                        "Unknwon element {:}".format(element) + "on the RHS."
                    )
...
if __name__ == "__main__":
    ...
    methaneDecomposition = Reaction(
        name="Methane decomposition",
        LHS={methane: 1},
        RHS={carbon: 1, hydrogen: 2},
    )
